$(function () {
    ...
    $('#detail').click(function () {
        ...
        if (taskID != undefined) {
            return qBox.iFLoad({ title: '动火作业申请详细信息', src: 'fireTaskDetail.aspx?taskID=' + taskID + '&archive=' + archivedState + '&rdom=' + Math.random() + '&fatherCookie=' + getHtmlDocName(), w: 800, h: 460 });
        }
        else {
    ...
    $('#addApply').click(function () {
        //var taskID = $("input[.radio][checked]").val();
        //if (taskID == undefined) {
        //    return qBox.iFLoad({ title: '添加动火作业申请', src: 'fireTaskAdd.aspx?rdom=' + Math.random(), w: 800, h: 610 });
        //}
        //else { //复制式的添加
        //    return qBox.iFLoad({ title: '添加动火作业申请', src: 'fireTaskAdd.aspx?taskID=' + taskID + '&rdom=' + Math.random(), w: 800, h: 610 });
        //}
        return qBox.iFLoad({ title: '添加动火作业申请', src: 'fireTaskAdd.aspx?rdom=' + Math.random(), w: 800, h: 610 });
    });
    //修改事件
    $('#update').click(function () {
        ...
        var judge = "";
        $.post("Judge.ashx", { op: "fire", userName: userName, taskID: taskID }, function (data) {
            judge = data;
            if (taskID != undefined) {
                if (judge == "true") {
                    if (archivedState == "0") {
                        ...
                        if (state == "4" || state == "" || state == "5") {
                            return qBox.iFLoad({ title: '修改动火作业申请', src: 'fireTaskEdit.aspx?taskID=' + taskID + '&rdom=' + Math.random(), w: 800, h: 585 });
                        }
                        else {
                            alert("该记录不允许修改！");
                            return false;
                ...
                if(data == "true"){
                    if (archivedState == "0") {
                        ...
                        if (state == "4" || state == "5" || state == "" || state == "3") {
                            ...
                            if (confirm("您确定要删除吗?")) {
                                $.post("dataProcess.ashx", { op: "Fire_T", taskID: taskID, userName: userName, rdom: Math.random() }, function (data) {
                                    if (data == "") {
                                        $("#tr_" + taskID).remove();
                                    }
                                    else {
                ...
                if (data == "true") {
                    if (archivedState == "0") {
                        ...
                        if (state != "1" && state != "2" && state != "3") {
                            return qBox.iFLoad({ title: '提交动火作业申请', src: 'fireTaskSubmit.aspx?taskID=' + taskID + '&rdom=' + Math.random(), w: 800, h: 479 });
                        }
                        else {
                    ...
                    if (archivedState == "0") {
                        ...
                        if (state == "1") {
                            if (confirm("确定要撤回该申请吗？")) {
                                $.post("dataProcess.ashx", { op: "callbackFireTask_T", taskID: taskID, userName: userName, rdom: Math.random() }, function (data) {
                                    if (data == "") {
                                        alert("撤回成功！");
            ...
            if (taskID != undefined) {
                if (data == "true") {
                    if (archivedState == "0") {
                        if (state == "2" || state == "3") {
                            if (confirm("您确定要归档吗？")) {
                                $.post("dataProcess.ashx", { op: "archiveFireTask_T", taskID: taskID, userName: userName, rdom: Math.random() }, function (data) {
                                    if (data == "") {
                                        alert("归档成功！");
                                        $("#tr_" + taskID).remove();
                                        return;
    ...
    //    if (taskID != undefined) {
    //        if (state == "2" || state == "3") {
    //            return qBox.iFLoad({ title: '打印动火作业证', src: 'fireTaskPrint.aspx?taskID=' + taskID + '&rdom=' + Math.random(), w: 350, h: 87 });
    //        }
    //        else {
    //            alert("动火作业审批未开始或未结束，不可以打印！");
    //            return false;
    //        }
    //    }
    //    else {
            ...
            if (taskID != undefined) {
                if (data == "true") {
                    if (state == "2" || state == "3") {
                        ...
                        alert("动火作业审批未开始，不可以打印！");
                    } else if (state == "1") {
                        alert("动火作业正在审批中，不可以打印！");
                    } else if (state == "4") {
                        alert("动火作业已撤回，不可以打印！");
                    } else if (state == "5") {
                        alert("动火作业已退回，不可以打印！");
    ...
    $('#history').click(function () {
        ...
        //alert(taskID);
        if (taskID != undefined) {
            return qBox.iFLoad({ title: '动火作业批复记录', src: 'taskHistoryApproveMan.aspx?categoryNum=247&taskID=' + taskID + '&rdom=' + Math.random(), w: 900, h: 250 });
        }
        else {
            alert("请选择您要查看的信息！");
            return false;

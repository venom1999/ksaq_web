$(function () {
    ...
    $('#detail').click(function () {
        ...
        if (confID != undefined && confID != "") {
            return qBox.iFLoad({ title: '系统管理评审会议记录详细信息', src: 'sysAppConfDetail.aspx?confID=' + confID + '&rdom=' + Math.random(), w: 350, h: 414 });
        }
        else {
            alert("请选择您要查看的记录！");
            return false;
    ...
    $("#popAdd").click(function () {
        return qBox.iFLoad({ title: '添加系统管理评审会议记录信息', src: 'sysAppConfAdd.aspx?rdom=' + Math.random(), w: 350, h: 414 });
    });
    ...
    $("#popEdit").click(function () { //state 为 1 表示审查开始之前，为2表示审查结束，为3表示在审查之中
        var confID = $("input[.radio][checked]").val();
        if (confID != undefined && confID != "") {
            return qBox.iFLoad({ title: '修改系统管理评审会议记录信息', src: 'sysAppConfEdit.aspx?confID=' + confID + '&rdom=' + Math.random(), w: 350, h: 414 });
        }
        else {
            alert("请选择您要修改的记录！");
            return false;
    ...
    $('#delete').click(function () {
        ...
        if (confID != undefined && confID != "") {
            if (confirm("您确定要删除吗？")) {
                $.post("dataProcess.ashx", { op: "sysAppConfDel", confID: confID, rdom: Math.random() }, function (data) {
                    if (data == "") {
                        $("#tr_" + confID).remove();
                    }
                    else {
    ...
    $('#btnReset').click(function () {
        $('#txt_ConfTime').attr("value", "");
        $('#txt_Place').attr("value", "");
        $('#txt_ChairMan').attr("value", "");
        $('#txt_Participants').attr("value", "");
        $('#txt_Record').attr("value", "");
        $('#txt_fillPer').attr("value", "");
        $('#txt_fillTime').attr("value", "");
        $('#txt_Contents').attr("value", "");
    });

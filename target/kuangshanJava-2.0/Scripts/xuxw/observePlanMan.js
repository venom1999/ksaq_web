$(function () {
    ...
    $('#detail').click(function () {
        ...
        if (planID != undefined && planID != "") {
            return qBox.iFLoad({ title: '任务观察计划详细信息', src: 'observePlanDetail.aspx?planID=' + planID + '&rdom=' + Math.random(), w: 350, h: 345 });
        }
        else {
            alert("请选择您要查看的记录！");
            return false;
    ...
    $("#popAdd").click(function () {
        return qBox.iFLoad({ title: '添加任务观察计划信息', src: 'observePlanAdd.aspx?rdom=' + Math.random(), w: 350, h: 345 });
    });
    ...
    $("#popEdit").click(function () { //state 为 1 表示审查开始之前，为2表示审查结束，为3表示在审查之中
        var planID = $("input[.radio][checked]").val();
        if (planID != undefined && planID != "") {
            return qBox.iFLoad({ title: '修改任务观察计划信息', src: 'observePlanEdit.aspx?planID=' + planID + '&rdom=' + Math.random(), w: 350, h: 345 });
        }
        else {
            alert("请选择您要修改的记录！");
            return false;
    ...
    $('#delete').click(function () {
        ...
        if (planID != undefined && planID != "") {
            if (confirm("您确定要删除吗？")) {
                $.post("dataProcess.ashx", { op: "observePlanDel", planID: planID, rdom: Math.random() }, function (data) {
                    if (data == "") {
                        $("#tr_" + planID).remove();
                    }
                    else {
